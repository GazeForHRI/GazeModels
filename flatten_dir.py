...
def _unflatten_from_new_csv(output_dir: str, base_dir: str, reader: csv.DictReader):
    ...
    for row in reader:
        idx = row["id"]
        exp_dir_rel = row["exp_dir_relpath"]
        item_rel = row["item_relpath"]
...
def _unflatten_from_old_csv(output_dir: str, base_dir: str, reader: csv.DictReader):
    ...
    Legacy behavior:
      - Each row corresponds to ONE experiment bucket with a directory destination.
      - The flattened bucket lives in <output_dir>/<id> and contains *subdirectories*
        (e.g., gaze model folders). We copy each subdirectory back into
        <base_dir>/<relative_path>/<subdir>.
    This does NOT support single files; it's intentionally for the old gaze-estimation layout.
    """
    for row in reader:
        idx = row["id"]
        ...
        for name in os.listdir(src_dir):
            ...
            # Old script used directory copy for models. Preserve that behavior,
            # but allow accidental files too.
...
if __name__ == "__main__":
    ...
    MODELS_TO_INCLUDE = ["puregaze_gaze360_rectification"]
    OUTPUT_DIR_GAZE = f"{BASE_DIR}/flattened_gaze_estimations (puregaze_gaze360_rectification)"
    ...
    # ---------------- Blink annotations ----------------
    # OUTPUT_DIR_BLINK_ANNOTATIONS = f"{BASE_DIR}/flattened_blink_annotations/flattened_blink_annotations_by_deniz_04_half1"
    # ANNOTATORS_TO_INCLUDE = ["deniz"]
    # flatten_blink_annotations(
    #     SUBJECT_DIRS, EXP_TYPES, OUTPUT_DIR_BLINK_ANNOTATIONS,
    #     annotators_to_include=ANNOTATORS_TO_INCLUDE, base_dir=BASE_DIR
    # )
    # unflatten(OUTPUT_DIR_BLINK_ANNOTATIONS, base_dir=BASE_DIR)
