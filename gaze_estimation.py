    ...
    def estimate(self, rgb_video: List[np.ndarray], timestamps: np.ndarray, bboxes: np.ndarray, output_size=(224, 224), padding=False, exp_dir = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if(self.rectification):
            try:
                ...
                normalized_frames = []
                max_rect_idx = eye_positions_cam.shape[0] - 1
                for i, frame in enumerate(tqdm(rgb_video, desc="Gaze estimation with model: " + self.get_model_name() + "(RECTIFICATION)")):
                    ...
                    # Logic: If video has 1 extra frame (i > max), reuse the last valid data. 
                    # If it has more than 1 extra frame, raise an error.
                    idx_to_use = i
                    if i > max_rect_idx:
                        if i == max_rect_idx + 1:
                            idx_to_use = max_rect_idx # Reuse last frame's metadata
                        else:
                            len_rgb_video = len(rgb_video)
                            raise IndexError(f"Video frame index {i} exceeds rectification data size {max_rect_idx + 1} {len_rgb_video}")
                    ...
                    center_i = convert_to_opencv_basis(eye_positions_cam[idx_to_use, 1:4])*1000     
                    target_i = convert_to_opencv_basis(target_positions_cam[idx_to_use, 1:4])*1000  
                    rvec_i   = rotate_rvec_z_axis(flip_yaw_180(convert_to_opencv_basis(headrotvectors[idx_to_use])),-90)           
