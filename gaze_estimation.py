		...
		if(self.rectification):
			try:
				...
				for i, frame in enumerate(tqdm(rgb_video, desc="Gaze estimation with model: " + self.get_model_name() + "(RECTIFICATION)")):
					...
					if i == 0 and exp_dir is not None: # for debug purposes
						save_path = os.path.join("./", f"first_input_{self.get_model_name()}.png")
						# We convert RGB -> BGR so OpenCV saves the colors correctly
						cv2.imwrite(save_path, cv2.cvtColor(norm_img, cv2.COLOR_RGB2BGR))
