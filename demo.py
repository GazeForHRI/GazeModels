...
class Gaze3DDemo:
    ...
    def __draw_prediction_video(
        ...
        """
        Load the video and the detected head bbox and draw the bbox/gaze on the video.
        Writes the result using OpenCV's VideoWriter (no ffmpeg).
        """
        # Load predictions
        ...
        if not ret or frame is None:
            cap.release()
            raise RuntimeError(f"Failed to read first frame from {self.input_filename}")
        ...
        writer = cv2.VideoWriter(self.output_file, fourcc, fps, (img_w, img_h), True)
        if not writer.isOpened():
            cap.release()
            raise RuntimeError(f"Failed to open VideoWriter for {self.output_file}")
        ...
        with tqdm(total=frame_count, desc="draw heads bbox") as pbar:
            while ret and frame is not None:
                ...
                # ===== Draw (keep same behavior: BGR -> RGB for drawing) =====
                frame_rgb = frame[..., ::-1].copy().astype(np.uint8)  # BGR -> RGB

                rows = predicted_gaze[predicted_gaze["frame_id"] == frame_nb]
                ...
                # ===== Write (convert back to BGR for OpenCV encoder) =====
                frame_bgr_out = frame_rgb[..., ::-1]
